		...
		<div class="grid grid-rows-1 sm:grid-cols-3 md:grid-cols-6 xl:grid-cols-8 gap-5">
			{#if !click}
				{#each $datas as movies}
					<div>
						<button
							class="flex flex-col bg-white w-full p-2 items-center justify-center md:justify-start rounded-xl text-center hover:bg-gray-300 transition"
						>
							<img class="rounded-xl object-contain" src={movies.image} alt="" />
